...
public class serverUDP {
	...
	public static String testUserHash(byte[] data, String nonce, String address, int port){
		...
		String user = "";
		String hash = "";
		connStatus con = connections.get(port);
	...
	public static void main(String[] args){
		...
		for(;;){
			...
			try {
				...
				if(data.charAt(0) == ('Y') && !connections.containsKey(packet.getPort())){
				...
				else if(connections.containsKey(packet.getPort()) && connections.get(packet.getPort()).getAuth()){
					...
					System.out.println(connections.containsKey(packet.getPort()));
